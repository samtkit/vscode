...
import {
  ...
  TextmateGrammar,
  Pattern,
} from "../generated/textmateGrammar";
...
suite("TextMate Grammar Test Suite", () => {
  ...
  test("#code is included in all patterns", async () => {
    ...
    function checkPatternRecursive(pattern: Pattern) {
      if (
        (pattern.begin == null && pattern.end == null) ||
        isStringOrComment(pattern)
      ) {
        return;
      }
      assert(
  ...
  test("scope names end with .samt", async () => {
    function checkPatternRecursive({
      name,
      patterns,
      captures,
      beginCaptures,
  ...
  test("no unused values in repositories", async () => {
    ...
    const encounteredKeys = new Set([
      ...findUsedPatterns(grammar.patterns),
      ...findUsedPatterns(Object.values(grammar.repository)),
    ]);
